...
import { assertEstimatedRerankRequestBudget } from "@/lib/billing/preflight";
...
import { executeRerank } from "@/lib/rerank/service";
import { parseRerankRequest } from "@/lib/rerank/validation";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const input = parseRerankRequest(payload);
    ...
    await assertEstimatedRerankRequestBudget(auth.userId, input);

    const result = await executeRerank(input, authContext);
